...
function writeRustMetadata({ outputRoot, atlases }) {
  const splitRoot = join(outputRoot, "phosphor_icons");
  rmSync(splitRoot, { recursive: true, force: true });
  mkdirSync(splitRoot, { recursive: true });

  const header = "// @generated by tools/icon-atlas/generate-phosphor-icons.mjs";
  const includePath = (fileName) =>
    `include!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/icons/phosphor/phosphor_icons/${fileName}"));`;
  const writeRustFile = (filePath, lines) => {
    writeFileSync(filePath, `${[header, "", ...lines].join("\n")}\n`);
  };

  writeRustFile(join(outputRoot, "phosphor_icons.rs"), [
    includePath("types.rs"),
    includePath("atlases.rs"),
    includePath("entries.rs"),
    includePath("lookup.rs"),
  ]);
  ...
  typeLines.push(...rustEnumVariants());
  typeLines.push("}");
  typeLines.push("");
  typeLines.push("pub(crate) struct PhosphorAtlasEntry {");
  typeLines.push("    pub image: ImageId,");
  typeLines.push("    pub physical_size: u32,");
  typeLines.push("    pub width: u32,");
  typeLines.push("    pub height: u32,");
  typeLines.push("    pub bytes: &'static [u8],");
  typeLines.push("}");
  typeLines.push("");
  typeLines.push("pub(crate) struct PhosphorIconEntry {");
  typeLines.push("    pub icon: PhosphorIcon,");
  typeLines.push("    pub image: ImageId,");
  typeLines.push("    pub symbol: &'static str,");
  typeLines.push("    pub source_name: &'static str,");
  typeLines.push("    pub logical_size: u32,");
  typeLines.push("    pub physical_size: u32,");
  typeLines.push("    pub atlas: ImageId,");
  typeLines.push("    pub source: Rect,");
  typeLines.push("}");
  typeLines.push("");
  typeLines.push(`pub(crate) const DENSE_ICON_LOGICAL_SIZE: u32 = ${LOGICAL_ICON_SIZES[0]};`);
  typeLines.push(`pub(crate) const STANDARD_ICON_LOGICAL_SIZE: u32 = ${LOGICAL_ICON_SIZES[1]};`);
  typeLines.push(`pub(crate) const ICON_ATLAS_PADDING: u32 = ${PADDING};`);
  typeLines.push(`pub(crate) const ICON_ATLAS_COLUMNS: u32 = ${COLUMNS};`);
  typeLines.push(`pub(crate) const ICON_COUNT: usize = ${ICONS.length};`);
  writeRustFile(join(splitRoot, "types.rs"), typeLines);

  const atlasLines = [];
  for (const { physicalSize, width, height } of atlases) {
    atlasLines.push(`pub(crate) const ICON_ATLAS_${physicalSize}: ImageId = ImageId::from_raw(${atlasId(physicalSize)});`);
    atlasLines.push(`pub(crate) const ICON_ATLAS_${physicalSize}_WIDTH: u32 = ${width};`);
    atlasLines.push(`pub(crate) const ICON_ATLAS_${physicalSize}_HEIGHT: u32 = ${height};`);
    atlasLines.push(`pub(crate) const ICON_ATLAS_${physicalSize}_BYTES: &[u8] = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/icons/phosphor/atlas-${physicalSize}.rgba"));`);
  }
  atlasLines.push("");
  atlasLines.push("pub(crate) const ICON_ATLASES: &[PhosphorAtlasEntry] = &[");
  for (const { physicalSize, width, height } of atlases) {
    atlasLines.push("    PhosphorAtlasEntry {");
    atlasLines.push(`        image: ICON_ATLAS_${physicalSize},`);
    atlasLines.push(`        physical_size: ${physicalSize},`);
    atlasLines.push(`        width: ${width},`);
    atlasLines.push(`        height: ${height},`);
    atlasLines.push(`        bytes: ICON_ATLAS_${physicalSize}_BYTES,`);
    atlasLines.push("    },");
  }
  atlasLines.push("];");
  writeRustFile(join(splitRoot, "atlases.rs"), atlasLines);

  const entryLines = [];
  entryLines.push("#[rustfmt::skip]");
  entryLines.push("pub(crate) const ICON_ENTRIES: &[PhosphorIconEntry] = &[");
  for (const [variant, symbol, sourceName] of ICONS) {
    ...
    for (const logicalSize of LOGICAL_ICON_SIZES) {
      for (const scaleFactor of SCALE_FACTORS) {
        ...
        entryLines.push(
          `    PhosphorIconEntry { icon: PhosphorIcon::${variant}, image: ImageId::from_raw(${imageId(iconIndex, logicalSize, physicalSize)}), symbol: "${symbol}", source_name: "${sourceName}", logical_size: ${logicalSize}, physical_size: ${physicalSize}, atlas: ICON_ATLAS_${physicalSize}, source: Rect::new(${x}.0, ${y}.0, ${physicalSize}.0, ${physicalSize}.0) },`,
        );
      }
    }
  }
  entryLines.push("];");
  writeRustFile(join(splitRoot, "entries.rs"), entryLines);

  writeRustFile(join(splitRoot, "lookup.rs"), [
