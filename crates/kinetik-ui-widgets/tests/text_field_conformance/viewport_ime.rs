...
use kinetik_ui_text::{TextComposition, TextEditState, TextLayoutKey, TextLayoutStore, TextStyle};
...
fn text_transform(output: &kinetik_ui_core::FrameOutput) -> Transform {
    output
        .primitives
        .iter()
...
fn blink_off_caret_still_publishes_native_ime_geometry() {
    ...
    let mut state = TextEditState::new("abc");
    state.set_caret(0);
    let context = kinetik_ui_core::FrameContext::new(
        ViewportInfo::new(
    ...
    assert!(!output.primitives.iter().any(|primitive| matches!(
        primitive,
        Primitive::Rect(rect)
            if rect.fill == Some(kinetik_ui_core::Brush::Solid(caret_color))
                && rect.rect.width <= 1.0
    )));
}
...
fn shaped_geometry_reuses_one_layout_for_hit_selection_paint_caret_and_ime() {
    ...
    assert_eq!(
        state.selection,
        kinetik_ui_text::TextSelection::new(0, target_offset)
    );
