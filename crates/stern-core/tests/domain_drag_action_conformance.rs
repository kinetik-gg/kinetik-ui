...
fn release_outcome_at(end: Point) -> stern_core::CapturedDomainDragGesture {
...
fn release_actions(gesture: &stern_core::CapturedDomainDragGesture) -> Vec<(Option<usize>, bool)> {
    gesture
        .actions
        .iter()
...
fn captured_in_source_scope(
    ui: &mut stern_core::Ui<'_>,
    id: WidgetId,
) -> stern_core::CapturedDomainDragGesture {
...
fn legacy_and_disabled_pre_resolver_cancellation_reaches_domain_only() {
    ...
    assert_eq!(
        ...
        stern_core::SelectionGesturePhase::Cancel
    );
