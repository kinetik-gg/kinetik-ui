...
fn run_drag(harness: &mut UiTestHarness) -> stern_core::Response {
    harness
        .run_frame(|ui| {
            let id = ui.id("drag");
...
fn run_press(harness: &mut UiTestHarness) -> stern_core::Response {
    harness
        .run_frame(|ui| {
            let id = ui.id("press");
            let (input, memory) = ui.input_and_memory_mut();
...
fn planned_drop_rejects_release_outside_the_captured_source_clip() {
    ...
    for target_first in [false, true] {
        ...
        assert_eq!(routes.drop, stern_core::PointerRoute::Blocked);
        assert_eq!(drop.source, None);
        assert!(!drop.dropped);
        assert_eq!(harness.memory().released_drag_source(), None);
    }
...
fn planned_drop_uses_first_release_geometry_in_both_evaluation_orders() {
    ...
    for target_first in [false, true] {
        ...
        assert_eq!(
            routes.drop,
            stern_core::PointerRoute::Target(drop.response.id)
        );
        assert!(drop.dropped);
    }
...
fn same_frame_planned_drag_uses_press_and_release_probes_not_final_snapshot() {
    ...
    for target_first in [false, true] {
        ...
        let ((source, drop), output) = harness.run_frame(|ui| {
            ...
            assert_eq!(routes.ordinary, stern_core::PointerRoute::Target(source));
            assert_eq!(routes.wheel, stern_core::PointerRoute::Target(wheel));
...
fn drop_uses_canonical_release_geometry_and_rejects_missing_event_position() {
    let source = stern_core::WidgetId::from_key("source");
    let target = stern_core::WidgetId::from_key("target");
...
fn plain_pointer_capture_release_cleans_without_synthesizing_a_click() {
    let mut harness = UiTestHarness::new();
    let owner = stern_core::WidgetId::from_key("plain-capture");
