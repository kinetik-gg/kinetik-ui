...
fn ordered_pointer_events_localize_individually_and_clips_keep_only_release_cleanup() {
    ...
    *harness.input_mut() = input;
    ...
    assert_eq!(
        ...
        Err(stern_core::InputStreamConflict::Pointer),
...
fn spatial_localization_never_heals_a_root_pointer_projection_conflict() {
    ...
    assert_eq!(conflict, Err(stern_core::InputStreamConflict::Pointer));
    ...
    assert!(matches!(
        ...
        [stern_core::FrameWarning::InputStreamConflict {
            conflict: stern_core::InputStreamConflict::Pointer
        }]
    ));
}
