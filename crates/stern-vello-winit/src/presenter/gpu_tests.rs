...
use stern_vello::{VelloNativeTextureRegistry, VelloNativeTextureScope};
use vello::{
...
fn source_descriptor() -> TextureDescriptor<'static> {
    TextureDescriptor {
        label: Some("stern-rend-04b-producer"),
        size: Extent3d {
            width: 1,
            height: 1,
            depth_or_array_layers: 1,
...
fn create_target(presenter: &VelloWindowPresenter) -> Texture {
    ...
    gpu.context.devices[gpu.dev_id]
        ...
        .create_texture(&TextureDescriptor {
            label: Some("stern-rend-04b-target"),
            size: Extent3d {
                width: WIDTH,
                height: HEIGHT,
                depth_or_array_layers: 1,
...
fn read_center(
    ...
    let buffer = device.create_buffer(&BufferDescriptor {
        label: Some("stern-rend-04b-readback"),
        size: u64::from(PADDED_BYTES_PER_ROW) * u64::from(HEIGHT),
        usage: BufferUsages::COPY_DST | BufferUsages::MAP_READ,
        mapped_at_creation: false,
    });
    let mut encoder = device.create_command_encoder(&CommandEncoderDescriptor {
        label: Some("stern-rend-04b-copy"),
    });
